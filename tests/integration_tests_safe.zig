//! Safe Integration Tests (No quantifiers or alternation)
//!
//! Tests that should work without SPLIT operations
...
// =============================================================================
// Metacharacters (Safe - without quantifiers)
// =============================================================================
...
test "Integration: multiple dots" {
    var re = try Regex.compile(std.testing.allocator, "a..z");
    defer re.deinit();

    try std.testing.expect(try re.test_("abcz"));
    try std.testing.expect(try re.test_("a12z"));
    try std.testing.expect(!try re.test_("az"));
    try std.testing.expect(!try re.test_("abz"));
}

// =============================================================================
// Groups and Captures (Safe - without quantifiers)
// =============================================================================
...
test "Integration: convenience test function" {
    try std.testing.expect(try zregexp.test_(std.testing.allocator, "quick", "quick test"));
    try std.testing.expect(!try zregexp.test_(std.testing.allocator, "slow", "quick test"));
}

test "Integration: convenience find function" {
    ...
    try std.testing.expectEqual(@as(usize, 10), find_result.?.start);
}

test "Integration: convenience findAll function" {
